...
def _cap_segment_prefix(segment: list[Message], min_segment: int) -> list[Message]:
    ...
    Крайний случай (гигантские сообщения): даже min_segment сообщений не
    влезают в бюджет — они включаются КАК ЕСТЬ, без усечения текстов.
    Сверхбольшой prompt уходит во внутреннюю модель; при неудаче сводки
    boundary не двигается — безопасный отказ, сегмент повторится
    ...
    if len(included) >= min_segment:
        return included
    # Крайний случай: гигантские сообщения — берём min_segment как есть,
    # БЕЗ усечения текстов (усечения в коде нет). Prompt может превысить
    # бюджет; при неудаче сводки boundary не двигается (безопасный отказ).
